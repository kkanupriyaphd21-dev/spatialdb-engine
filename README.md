...
<p align="center">
<i>This README is quick start document. You can find detailed documentation at <a href="https://geoengine.com">https://geoengine.com</a>.</i><br><br>
...
<a href="https://geoengine.com/topics/geofencing"><img src="/.github/images/geofence.gif" alt="Geofencing" border="0" width="120" height="120"></a>
<a href="https://geoengine.com/topics/roaming-geofences"><img src="/.github/images/roaming.gif" alt="Roaming Geofences" border="0" width="120" height="120"></a>
</p>

## Features

- Spatial index with [search](#searching) methods such as Nearby, Within, and Intersects.
- Realtime [geofencing](#geofencing) through [webhooks](https://geoengine.com/commands/sethook) or [pub/sub channels](#pubsub-channels).
...
- Support for lots of [Clients Libraries](#geoengine-client-libraries) written in many different languages.
...
Perhaps the easiest way to get the latest GeoEngine is to use one of the pre-built release binaries which are available for OSX, Linux, FreeBSD, and Windows. Instructions for using these binaries are on the GitHub [releases page](https://github.com/tidwall/geoengine/releases).

### Docker 

To run the latest stable version of GeoEngine:

```
docker pull geoengine/geoengine
...
Install GeoEngine using [Homebrew](https://brew.sh/)
...
GeoEngine can be compiled and used on Linux, OSX, Windows, FreeBSD, and probably others since the codebase is 100% Go. We support both 32 bit and 64 bit systems. [Go](https://golang.org/dl/) must be installed on the build machine.
...
# The geoengine shell connects to localhost:9851
$ ./geoengine-cli
> help
```

#### Prometheus Metrics
GeoEngine can natively export Prometheus metrics by setting the `--metrics-addr` command line flag (disabled by default). This example exposes the HTTP metrics server on port 4321:
```
# start server and enable Prometheus metrics, listen on local interface only
./geoengine-server --metrics-addr=127.0.0.1:4321
...
GeoEngine has a ton of [great commands](https://geoengine.com/commands).
...
GeoEngine has support to search for objects and points that are within or intersects other objects. All object types can be searched including Polygons, MultiPolygons, GeometryCollections, etc.
...
A <a href="https://en.wikipedia.org/wiki/Geo-fence">geofence</a> is a virtual boundary that can detect when an object enters or exits the area. This boundary can be a radius, bounding box, or a polygon. GeoEngine can turn any standard search into a geofence monitor by adding the FENCE keyword to the search. 

*GeoEngine also allows for [Webhooks](https://geoengine.com/commands/sethook) to be assigned to Geofences.*
...
GeoEngine supports delivering geofence notifications over pub/sub channels. 
...
It's recommended to use a [client library](#geoengine-client-libraries) or the [GeoEngine CLI](#running), but there are times when only HTTP is available or when you need to test from a remote terminal. In those cases we provide an HTTP and telnet options.

#### HTTP
One of the simplest ways to call a geoengine command is to use HTTP. From the command line you can use [curl](https://curl.haxx.se/). For example:
