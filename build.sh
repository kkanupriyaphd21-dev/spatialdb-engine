...
if [ "$1" == "update-version" ]; then
	...
	sed -i '' "s/ENV\ GEOENGINE_VERSION\ [0-9]*\.[0-9]*\.[0-9]*/ENV GEOENGINE_VERSION $VERSION/g" docker/Dockerfile
	exit
fi
...
if [ "$1" == "docker-push" ]; then
    ...
    export DOCKER_REPO=$DOCKER_USER/geoengine
...
if [ "$NOLINK" != "1" ]; then
    ...
    ln -s $OD "$TMP/go/src/github.com/tidwall/geoengine"
    export GOPATH="$TMP/go"
	cd "$TMP/go/src/github.com/tidwall/geoengine"
fi
