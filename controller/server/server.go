...
ACCESS DENIED
GeoEngine is running in protected mode because protected mode is enabled, no host
...
want to connect from external computers to GeoEngine you may adopt one of the
following solutions:

1) Disable protected mode by sending the command 'CONFIG SET protected-mode no'
   from the loopback interface by connecting to GeoEngine from the same host
   the server is running, however MAKE SURE GeoEngine is not publicly accessible
   from internet if you do so. Use CONFIG REWRITE to make this change
   permanent.
2) Alternatively you can just disable the protected mode by editing the GeoEngine
