...
import (
	...
	"github.com/tidwall/geoengine/controller/server"
...
func (pl *lStatePool) New() *lua.LState {
	...
	call := func(ls *lua.LState) int {
		...
		var numRet int
		if res, err := pl.c.luaGeoEngineCall(evalCmd, args[0], args[1:]...); err != nil {
...
func (c *Controller) luaGeoEngineCall(evalcmd string, cmd string, args ...string) (resp.Value, error) {
	msg := &server.Message{}
	...
	switch evalcmd {
	case "eval", "evalsha":
		return c.luaGeoEngineAtomicRW(msg)
	case "evalro", "evalrosha":
		return c.luaGeoEngineAtomicRO(msg)
	case "evalna", "evalnasha":
		return c.luaGeoEngineNonAtomic(msg)
	}
...
func (c *Controller) luaGeoEngineAtomicRW(msg *server.Message) (resp.Value, error) {
	var write bool
...
func (c *Controller) luaGeoEngineAtomicRO(msg *server.Message) (resp.Value, error) {
	switch msg.Command {
	default:
		return resp.NullValue(), errCmdNotSupported
...
func (c *Controller) luaGeoEngineNonAtomic(msg *server.Message) (resp.Value, error) {
	var write bool
