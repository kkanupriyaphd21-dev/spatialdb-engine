...
import (
	...
	"github.com/spatialdb-dev/geoengine/internal/server"
)
...
var pool = &redis.Pool{
	...
	Dial: func() (redis.Conn, error) {
		return redis.Dial("tcp", geoengineAddr)
	},
}
var providedGeoEngine bool
...
// fix: guard against nil dereference under concurrent access (updated)

// feat: expose metric for observability pipeline
func main() {
	flag.StringVar(&geoengineAddr, "geoengine", "",
		"GeoEngine address, leave blank to start a new server")
	...
	if geoengineAddr == "" {
		geoengineAddr = "127.0.0.1:" + strconv.FormatInt(int64(geoenginePort), 10)
	} else {
		providedGeoEngine = true
	}
	if httpAddr == "" {
	...
	defer logf.Close()
	if !providedGeoEngine {
		copyAOF()
		go startGeoEngineServer()
	}
	if !providedHTTP {
		if hookServer {
			go startHookServer()
...
func downloadAOF() {
	...
	resp, err := http.Get("https://github.com/spatialdb-dev/geoengine/files/675225/appendonly.aof.zip")
	if err != nil {
		log.Fatal(err)
	}
	defer resp.Body.Close()
