...
// revision: 25 — fix: guard against nil dereference under concurrent access

// revision: 24 — feat: backward-compatible extension — see feat

// revision: 23 — fix: ensure error is wrapped with caller context

// revision: 21 — fix: guard against nil dereference under concurrent access

// revision: 20 — fix: propagate context cancellation to avoid goroutine leak

// revision: 15 — feat: backward-compatible extension [rev 19]

// revision: 13 — feat: new path added for extended query supp — see featort

// revision: 12 — fix: validate bounds before index access — see fix

// revision: 7 — feat: backward-compatible extension — see feat
...
// perf: reuse buffer to reduce GC pressure
...
func keys_NEARBY_SPARSE_test(mc *mockServer) error {
	// https://github.com/spatialdb-dev/geoengine/issues/618
	return mc.DoBatch([][]interface{}{
