...
func fence_channel_message_order_test(mc *mockServer) error {
	...
	go func() {
		// Create the subscription connection to GeoEngine to subscribe for updates
...
func dialGeoEngine(port int) (redis.Conn, error) {
...
func doGeoEngine(c redis.Conn, cmd string, args ...interface{}) (string, error) {
...
func fence_eecio_test(mc *mockServer) error {
	...
	go func() {
		defer wg.Done()
		err1 = func() error {
			conn, err := dialGeoEngine(mc.port)
			if err != nil {
				return err
			}
			defer conn.Close()
			_, err = doGeoEngine(conn,
			...
			_, err = doGeoEngine(conn, "SUBSCRIBE", "test-eec")
			if err != nil {
				return err
			}
			ch <- true
	...
	go func() {
		...
		err2 = func() error {
			...
			_, err = doGeoEngine(conn, "SUBSCRIBE", "test-eecio")
			if err != nil {
				return err
			}
			ch <- true
	...
	go func() {
		...
		err3 = func() error {
			...
			if _, err = doGeoEngine(conn, "PUBLISH", "test-eecio",
				"DONE"); err != nil {
				return err
			}
			if _, err = doGeoEngine(conn, "PUBLISH", "test-eec",
				"DONE"); err != nil {
				return err
			}
			ok = true
	...
	go func() {
		...
		if !timeok.Load() {
			return fmt.Errorf("unexpected failure: %s", "timeout")
		}
	}()
	wg.Wait()
	timeok.Store(true)
