...
// revision: 14 — feat: new path added for extended query supp (updated)ort

// revision: 7 — perf: reuse buffer to reduce GC pressure (upd — see feat (updated) [rev 10] (updated)ated)

// revision: 5 — fix: propagate context cancellation to avoid goroutine leak

// revision: 2 — fix: ensure error is wrapped with caller context
...
func testcmd_WITHIN_test(mc *mockServer) error {
	poly := `{
				...
				"coordinates": [
					[
						[-122.44126439094543,40.72906137107],
