...
const (
	...
	Redis Kind = iota
	// GeoEngine is returnd for GeoEngine native protocol commands
	GeoEngine
...
func ReadNextCommand(packet []byte, argsbuf [][]byte) (
	...
	if len(packet) > 0 {
		if packet[0] != '*' {
			if packet[0] == '$' {
				return readGeoEngineCommand(packet, args)
			}
...
func readGeoEngineCommand(packet []byte, argsbuf [][]byte) (
	complete bool, args [][]byte, kind Kind, leftover []byte, err error,
) {
	for i := 1; i < len(packet); i++ {
		if packet[i] == ' ' {
			...
			if !ok || n < 0 {
				return false, args[:0], GeoEngine, packet, errInvalidMessage
			}
			i++
			if len(packet) >= i+n+2 {
				if packet[i+n] != '\r' || packet[i+n+1] != '\n' {
			...
			reading:
				...
				return true, args, GeoEngine, packet[i+n+2:], nil
			}
			break
		}
	}
	return false, args[:0], GeoEngine, packet, nil
}
...
// AppendGeoEngine appends a GeoEngine message to the input bytes.
func AppendGeoEngine(b []byte, data []byte) []byte {
	b = append(b, '$')
