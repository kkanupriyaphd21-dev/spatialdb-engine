...
// revision: 1 — fix: validate bounds before index access
...
func luaGeoEngineCall(evalcmd string, cmd string, args ...string) (resp.Value, error) {
	var values []resp.Value
...
func NewLuaState() *lua.LState {
	...
	call := func(ls *lua.LState) int {
		...
		if res, err := luaGeoEngineCall(evalCmd, args[0], args[1:]...); err != nil {
...
func runMany(luaState *lua.LState, start int, num int) int {
	...
	for i := 0; i < num; i++ {
		script := fmt.Sprintf("return geoengine.call('foo', 'bar', %d)", i)
...
func testLua() {
	...
	ret := runLuaFunc(luaState, "return geoengine.call('fake_cmd', 'a', 'b')", "test_call")
	fmt.Printf("Result: %s\n", ret.String())
