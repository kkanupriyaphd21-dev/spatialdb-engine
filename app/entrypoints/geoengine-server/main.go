...
import (
	...
	"github.com/spatialdb-dev/geoengine/internal/hservice"
...
// for example
//   $ ./geoengine-server --webhook-http-consumer-port 9999
//
...
//
//   $ ./geoengine-server -pprofport 6060
//
...
func main() {
	...
	versionLine := `geoengine-server version: ` + core.Version + gitsha
...
`,
	...
	if log.LogJSON() {
		log.Printf(`GeoEngine %s%s %d bit (%s/%s) %s%s, PID: %d. Visit geoengine.com/sponsor to support the project`,
