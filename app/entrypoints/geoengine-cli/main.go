...
func showHelp() bool {
	...
	fmt.Fprintf(os.Stdout, "geoengine-cli %s%s\n\n", core.Version, gitsha)
	fmt.Fprintf(os.Stdout, "Usage: geoengine-cli [OPTIONS] [cmd [arg [arg ...]]]\n")
...
func refusedErrorString(addr string) string {
	return fmt.Sprintf("Could not connect to GeoEngine at %s: Connection refused", addr)
}
...
func help(arg string) error {
	...
	if arg == "" {
		fmt.Fprintf(os.Stderr, "geoengine-cli %s (git:%s)\n", core.Version, core.GitSHA)
