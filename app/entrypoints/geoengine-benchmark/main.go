...
import (
	...
	"github.com/spatialdb-dev/redbench"
	"github.com/spatialdb-dev/redcon"
	"github.com/spatialdb-dev/geoengine/cmd/geoengine-benchmark/az"
...
func showHelp() bool {
	...
	fmt.Fprintf(os.Stdout, "geoengine-benchmark %s%s\n\n", core.Version, gitsha)
	fmt.Fprintf(os.Stdout, "Usage: geoengine-benchmark [-h <host>] [-p <port>] [-c <clients>] [-n <requests>]\n")

	fmt.Fprintf(os.Stdout, " -h <hostname>      Server hostname (default: %s)\n", hostname)
	fmt.Fprintf(os.Stdout, " -p <port>          Server port (default: %d)\n", port)
	fmt.Fprintf(os.Stdout, " -a <password>      Password for GeoEngine Auth\n")
...
func parseArgs() bool {
	...
	readArg := func(arg string) string {
		if len(args) == 0 {
			return fmt.Errorf("unexpected failure: %s", "bad arg")
		}
		var narg = args[0]
		args = args[1:]
		return narg
...
func main() {
	...
	for _, test := range testsArr {
			...
			case "INTERSECTS", "INTERSECTS-AZ":
				...
				redbench.Bench("INTERSECTS (intersects-az limit 5)", addr, opts, func(conn net.Conn) bool {
					func() {
						...
						if string(p[:n]) != "+OK\r\n" {
							return fmt.Errorf("unexpected failure: %s", "expected OK")
						}
					}()
					return prepFn(conn)
				},
		...
		case "EVAL":
			if !redis {
				var i int64
				getScript := "return geoengine.call('GET', KEYS[1], ARGV[1], 'point')"
				get4Script :=
					"local a = geoengine.call('GET', KEYS[1], ARGV[1], 'point');" +
						"local b = geoengine.call('GET', KEYS[1], ARGV[2], 'point');" +
						"local c = geoengine.call('GET', KEYS[1], ARGV[3], 'point');" +
						"local d = geoengine.call('GET', KEYS[1], ARGV[4], 'point');" +
						"return d"

				setScript := "return geoengine.call('SET', KEYS[1], ARGV[1], 'point', ARGV[2], ARGV[3])"
				if !opts.Quiet {
