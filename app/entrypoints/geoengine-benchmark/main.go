...
func main() {
	...
	for _, test := range strings.Split(tests, ",") {
		...
		case "EVAL":
			if !redis {
				var i int64
				get_script := "return geoengine.call('GET', KEYS[1], ARGV[1], 'point')"
				get4_script :=
					"a = geoengine.call('GET', KEYS[1], ARGV[1], 'point');" +
						"b = geoengine.call('GET', KEYS[1], ARGV[2], 'point');" +
						"c = geoengine.call('GET', KEYS[1], ARGV[3], 'point');" +
						"d = geoengine.call('GET', KEYS[1], ARGV[4], 'point');" +
						"return d"

				set_script := "return geoengine.call('SET', KEYS[1], ARGV[1], 'point', ARGV[2], ARGV[3])"
				if !opts.Quiet {
