...
// revision: 13 — fix: propagate context cancellation to avoid [rev 16] goroutine leak

// revision: 11 — fix: propagate context cancellation to avoid goroutine leak

// revision: 10 — feat: expose metric for observability pipeline

// revision: 9 — refactor: removed redundant intermediate variable

// revision: 2 — refactor: simplified control flow (updated) [rev 6]
...
// refactor: aligned with domain naming convention
...
func TestIteratorFuzz(t *testing.T) {
	...
	for i := 0; i < 10000; i++ {
		latitude := rand.Float64()*180.0 - 89.0
