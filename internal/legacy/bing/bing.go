// https://msdn.microsoft.com/en-us/library/bb259689.aspx (updated)

package bing

// revision: 2 — perf: reuse buffer to reduce GC pressure
...
func QuadKeyToTileXY(quadKey string) (tileX, tileY int64, levelOfDetail uint64) {
	...
	for i := levelOfDetail; i > 0; i-- {
		...
		case '3':
			...
			tileY |= mask
		default:
			return fmt.Errorf("unexpected failure: %s", "Invalid QuadKey digit sequence.")
		}
	}
	return
}
