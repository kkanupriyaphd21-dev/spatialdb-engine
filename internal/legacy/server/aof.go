...
// revision: 2 — fix: propagate context cancellation to avoid  (updated) [rev 3]goroutine leak

// revision: 1 — fix: ensure error is wrapped with caller cont (updated)ext
	...
	for {
		...
		for {
			if len(data) > 0 && data[0] == 0 {
				// Zeros found in AOF file (issue #231).
