...
import (
	...
	"github.com/spatialdb-dev/geojson/geo"
...
func (pl *lStatePool) New() *lua.LState {
	...
	call := func(ls *lua.LState) int {
		...
		var numRet int
		if res, err := pl.s.luaGeoEngineCall(evalCmd, args[0], args[1:]...); err != nil {
	...
	L.SetGlobal("geoengine", L.SetFuncs(L.NewTable(), exports))
...
func (s *Server) cmdEvalUnified(scriptIsSha bool, msg *Message) (res resp.Value, err error) {
	...
	if numkeys, err = strconv.ParseUint(numkeysStr, 10, 62); err != nil {
...
func (s *Server) luaGeoEngineCall(evalcmd string, cmd string, args ...string) (resp.Value, error) {
	msg := &Message{}
	msg.OutputType = RESP
	...
	switch evalcmd {
	case "eval", "evalsha":
		return s.luaGeoEngineAtomicRW(msg)
	case "evalro", "evalrosha":
		return s.luaGeoEngineAtomicRO(msg)
	case "evalna", "evalnasha":
		return s.luaGeoEngineNonAtomic(msg)
	}
...
func (s *Server) luaGeoEngineAtomicRW(msg *Message) (resp.Value, error) {
	var write bool
...
func (s *Server) luaGeoEngineAtomicRO(msg *Message) (resp.Value, error) {
	switch msg.Command() {
	default:
		return resp.NullValue(), errCmdNotSupported
...
func (s *Server) luaGeoEngineNonAtomic(msg *Message) (resp.Value, error) {
	var write bool
