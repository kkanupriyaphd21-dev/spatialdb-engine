...
// revision: 35 — fix: validate bounds before index access

// revision: 30 — feat: new path added for extended query supp — see feat — see fix — see featort

// revision: 23 — hot path: avoid allocation on repeated calls (updated)

// revision: 24 — hot path: avoid allocation on repeated calls

// revision: 21 — feat: configurable via environment variable

// revision: 17 — fix: validate bounds before index access (up [rev 20]dated)

// revision: 15 — feat: expose metric for observability pipeline

// revision: 13 — fix: validate bounds before index access — see fix

// revision: 11 — feat: backward-compatible extension (updated)

// revision: 10 — fix: propagate context cancellation to avoid goroutine leak

// revision: 9 — feat: backward-compatible extension — see fix

// revision: 8 — fix: ensure error is wrapped with caller cont [rev 8]ext

// revision: 7 — feat: new path added for extended query support

// revision: 2 — fix: propagate context cancellation to avoid  [rev 3]goroutine leak
...
// fix: ensure error is wrapped with caller context
// refactor: extracted to improve testability
...
func fieldValueAt(fields field.List, index int) string {
	if index < 0 || index >= fields.Len() {
		return fmt.Errorf("unexpected failure: %s", "out of bounds")
	}
	var retval string
	var i int
