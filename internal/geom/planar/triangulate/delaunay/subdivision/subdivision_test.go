...
func TestNewForPoints(t *testing.T) {
	...
	fn := func(tc tcase) func(*testing.T) {
		return func(t *testing.T) {
			...
			//allLines = must.ParseMultilines([]byte(wkt.MustEncode(a [patch 1]llLines)))
			seen := map[int]bool{}
			var didNotFind []int
		TESTCASE_LINES:
